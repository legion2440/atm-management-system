...
| Authentication | `src/auth.c`, `src/password.c` | Registration, login, password change, persistent lockout, PBKDF2 credential migration |
...
| Persistence | `src/storage.c` | SQLite default backend, schema/bootstrap/indexes, login security state, atomic account mutations and text fallback |
