# Bonus audit evidence

This file maps every bonus question from the official 01-edu audit to concrete implementation and executable evidence.

| Audit bonus | Implementation | Evidence |
| --- | --- | --- |
...
| Encrypted password storage | SHA-256 hashes are stored instead of plaintext passwords | `src/password.c`, `src/auth.c`, `tests/audit_flow.sh`, `tests/bonus_flow.sh` |
...
| Own Makefile | Warning-clean C11 build, tests, sanitizer target and optional text-only build | `Makefile` |
...
```text
interest/date tests: mandatory interest formulas
audit flow: official mandatory scenarios against SQLite
bonus flow: SQLite schema/FK/index + TUI + password change + summary + text fallback
notification flow: two-session instant ownership-transfer notification
```

Memory/undefined-behavior check for the mandatory scenario:
